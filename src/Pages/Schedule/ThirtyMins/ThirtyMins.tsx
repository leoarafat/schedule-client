...
const ThirtyMins = () => {
    ...
    const { data: thirtyMinsAm, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch('http://localhost:5000/thirtyMinsAM');
            const data = res.json();
            return data;
        }
    })
    const { data: thirtyMinsPm } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch('http://localhost:5000/thirtyMinsPM');
            const data = res.json();
            return data;
        }
    })
            ...
            <div className='h-[25rem] md:py-0 py-12 px-2'>
                ...
                <div className='flex justify-center gap-4'>
                    ...
                    <div className='flex flex-col gap-4 h-[22rem] overflow-y-auto pr-2'>
                        ...
                        {thirtyMinsAm &&
                            thirtyMinsAm[0].slots.map((thirtyAm: any) => <span onClick={() => setSlot(thirtyAm)} key={thirtyAm._id}
                    ...
                    <div>
                        <div className='flex flex-col gap-4 h-[22rem] overflow-y-auto pr-2'>
                            {thirtyMinsPm &&
                                thirtyMinsPm[0].slots.map((thirtyPm: any) => <span onClick={() => setSlotPm(thirtyPm)} key={thirtyPm._id}
                                    className="cursor-pointer inline-block rounded border border-primary py-3 px-5 text-xl font-medium text-primary hover:bg-primary hover:text-white focus:outline-none focus:ring active:bg-primary"
                                >
                                    {thirtyPm}
                                </span>)
