    ...
    <>
      <div className="flex justify-center py-20">
        <div className="rounded-3xl border bg-gray-50 w-[30rem] flex flex-col gap-12 p-12">
          <h3 className="text-2xl font-semibold text-gray-700 text-center">Register To Your Account</h3>
          ...
          <form onSubmit={handleSubmit(handleRegister)} className="flex flex-col gap-8">
            {firebaseError && <p className="text-sm text-red-600 -mt-4">{firebaseError}</p>}
            <div className="w-full relative before:absolute before:bottom-0 before:h-0.5 before:left-0 before:origin-right focus-within:before:origin-left before:right-0 before:scale-x-0 before:m-auto before:bg-sky-400 focus-within:before:!scale-x-100 before:transition before:duration-300">
              <input
                ...
                type="text" placeholder="Your Name" className="w-full bg-transparent pb-3  border-b border-gray-300 outline-none transition p-2" />
            </div>
            {errors.displayName && <p className="text-sm text-red-600 -mt-4">{errors.displayName.message}</p>}
